...
def get_model(stub, op):
    feature = stub.GetModel(op)
    model = base64.b64decode(feature.model)
    return model

def send_model(stub, modelString, client_id):
    m = FederatedML_pb2.Model
    m.model = modelString
    m.id = client_id
    feature = stub.SendModel(m)
    if feature.value == 1:
        print("Model Sent")
    else:
...
def run():
    print("FL start!")
    print("-------------- Generate Data --------------")
    train_set, test_set, user_group = get_data(dataset)
    local_progress = LocalUpdate(dataset=train_dataset, idxs=user_groups)
    with grpc.insecure_channel('localhost:50051') as channel:
        stub = FederatedML_pb2_grpc.FederatedMLStub(channel)
        print("-------------- Initialize Model --------------")
        local_model = get_model(stub, 0)
        print("-------------- Start Train --------------")
        for i in range(1, global_round + 1):
            print("global_round: ", i)
            new_model, loss = local_progress.update_weights(model=copy.deepcopy(local_model), global_round=epoch)
            torch.save(new_model, 'tmp_model.pkl')
            with open('tmp_model.pkl', 'rb') as file:
                encoing_string = base64.b64encode(file.read())
                send_model(encoing_string, client_id)
            print("-------------- Get New Global Model--------------")
            local_model = get_model(stub, 1)
            print("-------------- Get New Global Model Successfully--------------")
